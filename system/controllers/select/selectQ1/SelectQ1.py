...
EXCHANGE_SELECTQ1 = "Exchange_selectq1"
...
class SelectQ1:
    ...
    def __init__(self):
        ...
        self.broker.create_fanout_and_bind(name_exchange =EXCHANGE_SELECTQ1, callback =self.callback_eof_calculator())
        self.handler_eof_games = HandlerEOF(broker =self.broker, node_id =self.id, target_name ="Games", total_nodes= self.total_nodes,
                                exchange_name =EXCHANGE_SELECTQ1, next_queues =[QUEUE_SELECTQ1_PLATFORMCOUNTER])
    ...
    def handler_filter_platforms(self):
        def handler_message(ch, method, properties, body):
            ...
            if result_dto.operation_type == OperationType.OPERATION_TYPE_EOF_INITIAL_DTO and result_dto.old_operation_type == ALL_REVIEWS_WAS_SENT:
                logging.info(f"Action: Recv Reviews EOF ! 📄📄📄 | result: success ✅") # delete el if y esta linea
