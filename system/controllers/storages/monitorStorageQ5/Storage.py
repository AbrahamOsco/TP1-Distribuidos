...
class Storage(DualInputNode):
    ...
    def send_result(self, client_id):
        values = sorted(self.list[client_id].items(), key=lambda item: (item[1], item[0]))
        ...
        for app_id, _ in values[index:]:
            games_to_send.append(GameIDNameDTO(app_id=app_id, name=self.games[client_id][app_id]))
