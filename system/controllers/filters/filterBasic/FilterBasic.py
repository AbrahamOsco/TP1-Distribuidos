...
class FilterBasic:
    ...
    def filter_fields_empty_games(self, gamesDTO):
        games_filtered = []
        for a_game_dto in gamesDTO.games_dto:
            if a_game_dto.name != "" and a_game_dto.genres != "":
                games_filtered.append(a_game_dto)
        return games_filtered

    def filter_field_empty_reviews(self, reviewsDTO):
        reviews_filetered = []
        for a_review_dto in reviewsDTO.reviews_dto:
            if a_review_dto.review_text != "":
                reviews_filetered.append(a_review_dto)
        return reviews_filetered
